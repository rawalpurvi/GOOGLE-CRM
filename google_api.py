...
def user_contacts():
    ...
    try:
        ...
        results = service.people().connections().list(
            ...
            pageSize=25,
            personFields='metadata,names,emailAddresses,phoneNumbers,addresses').execute()
...
def delete_google_contact(contactResourceName):
    ...
    try:
        ...
        service.people().deleteContact(resourceName='people/'+contactResourceName).execute()
...
'''
def get_single_contact(contactResourceName):
    #Get credential from session
    credentials = Credentials(
                        **session['credentials'])
    session['credentials'] = credentials_to_dict(credentials)
    ...
    try:
        ...
        results = service.people().get(resourceName='people/'+contactResourceName,
...
def update_google_contact(contactResourceName,data):
    ...
    try:
        ...
        result = service.people().updateContact(resourceName='people/'+contactResourceName,
            updatePersonFields='names,emailAddresses,phoneNumbers,addresses',body=updateContactInfo).execute()
    except:
        return redirect(url_for('index', _external=True,_scheme='https'), code=302)
